...
name = ['left Pillar', 'right pillar', 'bottom slab', 'top slab', 'left cantilever', 'right cantilever', 'left kerb',
        'right kerb',
...
obj = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle',
       'triangle_1',
...
pos.append(
    [round(pos[0][0] + length[0] + length[3] + length[1] + length[5] - length[7], 4), round(pos[0][1] + height[0], 4)])
pos.append([round(pos[0][0] - length[4], 4), round(pos[0][1] + height[0] - height[4] - height[8], 4)])
pos.append(
    [round(pos[0][0] + length[0] + length[3] + length[1], 4), round(pos[0][1] + height[0] - height[5] - height[9], 4)])
pos.append([round(pos[0][0] + length[0], 4), round(pos[0][1] + height[0] - height[3] - height[10], 4)])
pos.append([round(pos[0][0] + length[0] + length[3] - length[11], 4),
            round(pos[0][1] + height[0] - height[3] - height[10], 4)])
...
df = pd.DataFrame([name, length, height, pos, area, centroid, moi, ah2, i_sum, obj],
                  index=['Name', 'Length', 'Height', 'Position', 'Area', 'Centroid', 'I', 'Ah2', 'I+Ah2',
                         'Object Type']).T
