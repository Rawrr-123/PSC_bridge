...
def calc_Ah2(area, centroid, composite_axis):
    return [round(area * (centroid[1] - composite_axis[1]) ** 2, 6),
            round(area * (centroid[0] - composite_axis[0]) ** 2, 6)]
...
class Cross_section:
    ...
    def length(self):
        if self.expansion_width > 0:
            return self.len + [self.len[10], round(self.len[2] * 0.45 - self.len[10] * 2, 5),
                               round(self.len[2] * 0.45 - self.len[10], 5), self.len[10], self.len[11],
                               round(self.len[2] * 0.45 - self.len[11], 5),
                               round(self.len[2] * 0.45 - self.len[11] * 2, 5), self.len[11]]
        else:
            return self.len
    ...
    def height(self):
        if self.expansion_width > 0:
            if self.expansion_width >= self.hei[10] and self.expansion_width >= self.hei[11]:
                ...
                h1 = h2 = 0
            return self.hei + [h1, h1, self.expansion_width - h1, self.expansion_width - h1, self.expansion_width - h2,
                               self.expansion_width - h2, h2, h2]
        else:
            return self.hei
    ...
    def name(self):
        nyam = ['left Pillar', 'right pillar', 'bottom slab', 'top slab', 'left cantilever', 'right cantilever',
                'left triangle', 'right triangle', 'left top fillet', 'right top fillet', 'left bottom fillet',
                'right bottom fillet']
        if self.expansion_width > 0:
            nyam = nyam + ['Expanded_triangle_left', 'Top_Expanded_rectangle_left', 'Side_Expanded_rectangle_left',
                           'Fillet_triangle_left', 'Fillet_triangle_right', 'Side_Expanded_rectangle_right',
    ...
    def obj(self):
        ...
        if self.expansion_width > 0:
            obui = obui + ['triangle_1', 'rectangle', 'rectangle', 'triangle_2', 'triangle_4', 'rectangle', 'rectangle',
                           'triangle_3']
        return obui
    ...
    def position(self):
        ...
        pos.append([round(pos[0][0] - self.length[4], 4),
                    round(pos[0][1] + self.height[0] - self.height[4], 4)])  # left cantilever  4
        pos.append([round(pos[0][0] + self.length[0] + self.length[3] + self.length[1], 4),
                    round(pos[0][1] + self.height[0] - self.height[5], 4)])  # right cantilever  5
        ...
        pos.append([round(pos[0][0] - self.length[4], 4),
                    round(pos[0][1] + self.height[0] - self.height[4] - self.height[6], 4)])  # left triangle  6
        pos.append([round(pos[0][0] + self.length[0] + self.length[3] + self.length[1], 4),
                    round(pos[0][1] + self.height[0] - self.height[5] - self.height[7], 4)])  # right triangle  7
        pos.append([round(pos[0][0] + self.length[0], 4),
                    round(pos[0][1] + self.height[0] - self.height[3] - self.height[9], 4)])  # left top fillet  8
        pos.append([round(pos[0][0] + self.length[0] + self.length[3] - self.length[10], 4),
                    round(pos[0][1] + self.height[0] - self.height[3] - self.height[9], 4)])  # right top fillet  9
        pos.append(
            [round(pos[0][0] + self.length[0], 4), round(pos[0][1] + self.height[2], 4)])  # left bottom fillet  10
        pos.append([round(pos[0][0] + self.length[0] + self.length[2] - self.length[11], 4),
                    round(pos[0][1] + self.height[2], 4)])  # right bottom fillet  11
        if self.expansion_width > 0:
...
class cables:
    ...
    def arrcoll(self):
        ...
        for i in range(nbot):
            if i == 0:
                bottompos.append(
                    [round(x + y, 4) for x, y in zip(self.section.position[0], [self.e + self.adash, self.b])])
                midbot.append(
                    [round(x + y, 4) for x, y in zip(self.section.position[0], [self.e + self.adash, self.b])])
            if i > 0:
...
class Dead_Load:
    def __init__(self, name, asum, sc, span, walkway_width, walkway_thickness, wearing_course, cw, railing_udl, I, ymax,
                 ymin):
        self.areasum = asum
        self.name = name
        self.sections = sc
...
def excel_export(section):
    df = pd.DataFrame(
        [section.name, section.length, section.height, section.position, section.section_area, section.section_centroid,
         section.section_moi, section.AH2, section.obj],
...
def excel_loads(PDL, ODL, PEDL, SIDL, sc, span):
    ...
    df5 = pd.DataFrame(
        {'Section at': sec_at, 'S+ PDL': strpdl1, 'S- PDL': strpdl2, 'S+ ODL': strodl1, 'S- ODL': strodl2,
    ...
    for i in range(len(sc)):
        x += PDL[i].load * 2
    a = [((x - PDL[0].load - PDL[-1].load) / ((len(sc) - 1) * 4)) * span,
         ((x - PDL[0].load - PDL[-1].load) / ((len(sc) - 1) * 4)) * span,
         2 * span * ((x - PDL[0].load - PDL[-1].load) / ((len(sc) - 1) * 4))]
    ...
    df4 = pd.DataFrame([a, b, c, d], columns=['RL', 'RR', 'Sum'],
                       index=['Dead Load', 'Other Load', 'Surface Load', 'Pedestrian load']
                       )
